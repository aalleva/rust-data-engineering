...
fn calculate_weights(years_active: &mut HashMap<String, i32>) -> HashMap<String, i32> {
    // Substract the creation year from the current year to get the number of years active
    ...
    for (language, &year) in years_active.iter() {
        let normalized_years = (year - min_year) as f64 / (max_year - min_year) as f64;
        let weight = (normalized_years * 99.0) as i32 + 1; // weight between 1 and 100
...
fn main() {
    let mut languajes: HashMap<String, i32> = init_languages();
    let weights = calculate_weights(&mut languajes);
